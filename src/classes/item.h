...
class Item {
  private:
    ...
    int _resources = 0; // how much resources can the player get from this item (in total)
    int _resources_left = 0;
    
    uint32_t _depleted_at = 0; // used to see if an item has depleted all it's resources and when it happend (-1 = not depleted, > 0 = depleted)
    uint32_t _last_action_at = 0; // used to see when an action was last taken (will be used for timing the deplition)


  public:
    Item(Window&, Renderer&, FPS&, Player&, const tile_type&, const int, int, int);
